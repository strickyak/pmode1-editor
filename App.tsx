...
const App: React.FC = () => {
  ...
  // Pre-calculate RGB values for current palette
  ...
  const updatePixels = useCallback((newData: Uint8Array) => {
    setRawImport(null);
  ...
  // Recompute pixels when weights or raw image changes
  useEffect(() => {
    if (rawImport) {
  ...
  const downloadPNG = useCallback(() => {
    ...
    link.download = 'pmode1_artwork.png';
    ...
    <div className="flex flex-col h-full bg-[#1e1e1e] select-none">
      ...
      <main className="flex flex-1 overflow-hidden">
        <Sidebar 
          ...
          setColorIdx={setCurrentColorIdx}
          isPermuteActive={isPermutationMode}
